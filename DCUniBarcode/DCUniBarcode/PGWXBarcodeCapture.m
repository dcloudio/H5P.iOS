...
@implementation ZXCapture(PGWXBarcode)
...
+ (PGWXBarcodeResult*)wxdecodeWithCGImage:(CGImageRef)imageToDecode withHints:(PGWXBarcodeHints*)hints error:(NSError**)error {
    ...
    if ( [filters isKindOfClass:[NSArray class]]
        ...
        for ( NSNumber *filter in filters ) {
            if ([filter isKindOfClass:NSNumber.class]) {
                PGWXBarcodeFormat type = (PGWXBarcodeFormat)[filter intValue];
                ZXBarcodeFormat zxbarcode = [ZXResult wxH5PForamt2ZX:type];
...
@implementation ZXResult(PGWXBarcode)
...
- (PGWXBarcodeFormat)wxscanBarcodeFormat {
    return [ZXResult wxZXForamt2H5P:self.barcodeFormat];
}
