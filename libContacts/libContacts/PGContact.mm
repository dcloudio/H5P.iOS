...
+ (NSDictionary*)defaultFields
{
    if (org_apache_cordova_contacts_defaultFields == nil) {
        org_apache_cordova_contacts_defaultFields = [NSDictionary dictionaryWithObjectsAndKeys:
                                                     ...
                                                     [NSNull null], kW3ContactDisplayName,
            ...
            if ([dict isKindOfClass:[NSDictionary class]]) {
                ...
                if ([value isKindOfClass:[NSString class]]) {
                    if (bUpdate && ([value length] == 0)) {
                        ...
                        //先测试是否是base64编码的数据
                        NSString *regex = @"data:image/.*base64,";
                        NSRange range = [value rangeOfString:regex options:NSRegularExpressionSearch];
                        if ( range.location != NSNotFound && range.location == 0 ) {
                            data = [NSData dataFromBase64String:[value substringFromIndex:range.length]];
                        } else {
                            NSString *fullPath = [PTPathUtil absolutePath:value];
                            if ( [fullPath length] ) {
                                NSURL *photoUrl = [NSURL fileURLWithPath:fullPath];
...
- (bool)setValue:(id)aValue forProperty:(ABPropertyID)aProperty inRecord:(ABRecordRef)aRecord asUpdate:(BOOL)bUpdate
{
    ...
    if (aValue && ![aValue isKindOfClass:[NSNull class]]) {
        ...
        bSuccess = ABRecordSetValue(aRecord, aProperty, ( CFTypeRef)aValue, &error);
        if (!bSuccess) {
...
- (bool)addToMultiValue:(ABMultiValueRef)multi fromDictionary:dict
{
    ...
    if (IS_VALID_VALUE(value)) {
        CFStringRef label = [PGContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
        bSuccess = ABMultiValueAddValueAndLabel(multi, ( CFTypeRef)value, label, NULL);
        if (!bSuccess) {
                ...
                if (IS_VALID_VALUE(val)) {
                    ...
                    if (i != kCFNotFound) {
                        if ([val length] == 0) {
                            ...
                            NSString* valueAB = ( NSString*)ABMultiValueCopyValueAtIndex(multi, i);
                            NSString* labelAB = ( NSString*)ABMultiValueCopyLabelAtIndex(multi, i);
                            if ((valueAB == nil) || ![val isEqualToString:valueAB]) {
                                ABMultiValueReplaceValueAtIndex(multi, ( CFTypeRef)val, i);
                            }
...
- (ABMultiValueRef)allocDictMultiValueFromArray:array forProperty:(ABPropertyID)prop
{
    ...
    for (NSDictionary* dict in array) {
        ...
        if (newDict) { // create a new dictionary with a Label and Value, value is the dictionary previously created
            ...
            NSString* typeValue = ((prop == kABPersonInstantMessageProperty) ? (NSString*)kABOtherLabel : addrType);
                ...
                if (idx != kCFNotFound) {
                    ...
                    for (id k in propArray) {
                        ...
                        if (bSet) {
                            ...
                            valueAB = [( NSDictionary*)existingDictionary valueForKey : [[PGContact defaultW3CtoAB] valueForKey:k]];
                            if (valueAB != nil) {
                    ...
                    if ([dict count] > 0) {
                        ...
                        ABMultiValueReplaceValueAtIndex(multi, ( CFTypeRef)dict, idx);
                    ...
                    if (dict) {
                        ...
                        // get the type out of the original dictionary for address
                        NSString* typeValue = ((prop == kABPersonInstantMessageProperty) ? (NSString*)kABOtherLabel : (NSString*)[field valueForKey:kW3ContactFieldType]);
                        // NSLog(@"typeValue: %@", typeValue);
                        [addDict setObject:typeValue forKey:kW3ContactFieldType];        //  im labels will be set as Other and address labels as type from dictionary
 ...
 */
- (NSDictionary*)toDictionary:(NSDictionary*)withFields allowNull:(BOOL)allowNull {
    return [self toDictionary:withFields allowNull:allowNull withAppContext:nil];
}
- (NSDictionary*)toDictionary:(NSDictionary*)withFields allowNull:(BOOL)allowNull withAppContext:(PDRCoreApp*)coreapp {
    ...
    if ([self.returnFields valueForKey:kW3ContactNickname]) {
        value = ( NSString*)ABRecordCopyValue(self.record, kABPersonNicknameProperty);
    ...
    if ([self.returnFields objectForKey:kW3ContactDisplayName] /*&& ((data == nil) || ([(NSDictionary*)data objectForKey : kW3ContactFormattedName] == [NSNull null]))*/) {
        ...
        id tryName = ( NSString*)ABRecordCopyCompositeName(self.record);
        if (tryName != nil) {
    ...
    value = [self extractMultiValue:kW3ContactPhoneNumbers allowNull:allowNull];
    if (value != nil) {
        [nc setObject:value forKey:kW3ContactPhoneNumbers];
    }
    // emails array
    // NSLog(@"getting emails");
    value = [self extractMultiValue:kW3ContactEmails allowNull:allowNull];
    if (value != nil) {
        [nc setObject:value forKey:kW3ContactEmails];
    }
    // urls array
    value = [self extractMultiValue:kW3ContactUrls allowNull:allowNull];
    if (value != nil) {
    ...
    if ([self.returnFields valueForKey:kW3ContactNote]) {
        // note
        value = ( NSString*)ABRecordCopyValue(self.record, kABPersonNoteProperty);
        [nc setObject:(value != nil) ? value:[NSNull null] forKey:kW3ContactNote];
    }
    
    if ([self.returnFields valueForKey:kW3ContactPhotos]) {
        value = [self extractPhotosWithAppContext:coreapp];
...
{
    return [self toDictionary:withFields allowNull:YES];
}
...
- (NSObject*)extractName
{
    ...
    for (NSString* i in fields) {
        if ([i isEqualToString:kW3ContactFormattedName]) {
            value = ( NSString*)ABRecordCopyCompositeName(self.record);
            ...
            value = ( NSString*)ABRecordCopyValue(self.record, (ABPropertyID)[[[PGContact defaultW3CtoAB] valueForKey:i] intValue]);
 ...
 * id
 */
- (NSObject*)extractMultiValue:(NSString*)propertyId allowNull:(BOOL)allowNull
{
    ...
    if (count) {
        ...
        for (CFIndex i = 0; i < count; i++) {
            ...
            if ([fields containsObject:kW3ContactFieldValue]) {
                value = ( NSString*)ABMultiValueCopyValueAtIndex(multi, i);
        ...
        valuesArray = allowNull ? [NSNull null]:@[];
    }
    if (multi) {
        CFRelease(multi);
    }
        ...
        for (CFIndex i = 0; i < count; i++) {
            ...
            for (id k in fields) {
                ...
                if (key && ![k isKindOfClass:[NSNull class]]) {
                    bFound = CFDictionaryGetValueIfPresent(dict, ( const void*)key, (const void**)&value);
                    if (bFound && (value != NULL)) {
                        CFRetain(value);
                        [newAddress setObject:( id)value forKey:k];
                        CFRelease(value);
                    } else {
...
- (NSObject*)extractIms
{
    ...
    if (count) {
        ...
        for (CFIndex i = 0; i < ABMultiValueGetCount(multi); i++) {
            ...
            if ([fields containsObject:kW3ContactFieldValue]) {
                // value = user name
                bFound =  CFDictionaryGetValueIfPresent(dict, kABPersonInstantMessageUsernameKey, (const void**)&value);
                if (bFound && (value != NULL)) {
                    CFRetain(value);
                    [newDict setObject:( id)value forKey:kW3ContactFieldValue];
                    CFRelease(value);
                } else {
            ...
            if ([fields containsObject:kW3ContactFieldType]) {
                bFound =  CFDictionaryGetValueIfPresent(dict, kABPersonInstantMessageServiceKey, (const void**)&value);
                if (bFound && (value != NULL)) {
                    CFRetain(value);
                    [newDict setObject:(id)[[PGContact class] convertPropertyLabelToContactType : ( NSString*)value] forKey:kW3ContactFieldType];
                    CFRelease(value);
                } else {
...
- (NSObject*)extractPhotosWithAppContext:(PDRCoreApp*)app
{
    ...
    if (ABPersonHasImageData(self.record)) {
        ...
        NSError* err = nil;
        NSString *filePath = [PTPathUtil uniqueNameInAppDocHasPrefix:@"photo" suffix:@"png" context:app] ;
        /*
        NSString* docsPath = [NSTemporaryDirectory()stringByStandardizingPath];
        NSError* err = nil;
        NSString* filePath = [NSString stringWithFormat:@"%@/photo_XXXXX", docsPath];
        char template1[filePath.length + 1];
        strcpy(template1, [filePath cStringUsingEncoding:NSASCIIStringEncoding]);
        mkstemp(template1);
        filePath = [[NSFileManager defaultManager]
                    stringWithFileSystemRepresentation:template1
                    length:strlen(template1)];*/
        
        // save file
        if ([data writeToFile:filePath options:NSAtomicWrite error:&err]) {
            NSString *retFilePath = [PTPathUtil relativePath:filePath withContext:app];
            photos = [NSMutableArray arrayWithCapacity:1];
            NSMutableDictionary* newDict = [NSMutableDictionary dictionaryWithCapacity:2];
            [newDict setObject:retFilePath ? retFilePath : filePath forKey:kW3ContactFieldValue];
...
- (BOOL)foundValue:(NSString*)testValue inFields:(NSDictionary*)searchFields
{
    ...
    if ( [searchFields valueForKey:kW3ContactDisplayName] ) {
        NSMutableString *displayName = [NSMutableString string];
       
        NSString* lastNameValue = ( NSString*)ABRecordCopyValue(self.record, kABPersonLastNameProperty);
        if ( lastNameValue ) {
            [displayName appendString:lastNameValue];
        }
        NSString* firstNameValue = ( NSString*)ABRecordCopyValue(self.record, kABPersonFirstNameProperty);
        if ( firstNameValue ) {
            [displayName appendString:firstNameValue];
        }
        bFound = [self testStringValue:testValue forSearchValue:displayName];
    }
    ...
    if ([searchFields valueForKeyIsArray:kW3ContactName]) {
        ...
        for (NSString* testItem in fields) {
            if ([testItem isEqualToString:kW3ContactFormattedName]) {
                NSString* propValue = ( NSString*)ABRecordCopyCompositeName(self.record);
                if ((propValue != nil) && ([propValue length] > 0)) {
                  ...
                  //  bFound = (range.location != NSNotFound);
                    bFound = [self testStringValue:testValue forSearchValue:propValue];
                    propValue = nil;
                }
            } else {
...
- (BOOL)testStringValue:(NSString*)testValue forW3CProperty:(NSString*)property
{
    ...
    if ([[PGContact defaultW3CtoAB] valueForKeyIsNumber:property]) {
        ...
        if (ABPersonGetTypeOfProperty(propId) == kABStringPropertyType) {
            NSString* propValue = ( NSString*)ABRecordCopyValue(self.record, propId);
            if ((propValue != nil) && ([propValue length] > 0)) {
                NSPredicate* containPred = [NSPredicate predicateWithFormat:@"SELF LIKE[cd] %@", testValue];
...
- (BOOL)testStringValue:(NSString*)testValue forSearchValue:(NSString*)searchTarget
{
    BOOL bFound = NO;
    if ((searchTarget != nil) && ([searchTarget length] > 0)) {
        @try {
            NSPredicate* containPred = [NSPredicate predicateWithFormat:@"SELF LIKE[cd] %@", testValue];
            bFound = [containPred evaluateWithObject:searchTarget];
        }
        @catch (NSException *e) {
            NSLog(@"%@",e);
        }
...
- (BOOL)testDateValue:(NSString*)testValue forW3CProperty:(NSString*)property
{
    ...
    if ([[PGContact defaultW3CtoAB] valueForKeyIsNumber:property]) {
        ...
        if (ABPersonGetTypeOfProperty(propId) == kABDateTimePropertyType) {
            NSDate* date = ( NSDate*)ABRecordCopyValue(self.record, propId);
            if (date != nil) {
    ...
    if (ABPersonGetTypeOfProperty(propId) == kABMultiStringPropertyType) {
        ...
        if (valueArray) {
            for (int i = 0; i < [valueArray count]; i++ ) {
                NSString *valuesAsString = [valueArray objectAtIndex:i];
                if ( valuesAsString ) {
...
- (BOOL)searchContactFields:(NSArray*)fields forMVDictionaryProperty:(ABPropertyID)propId withValue:(NSString*)testValue
{
    ...
    for (int i = 0; i < dictCount; i++) {
        CFDictionaryRef dict = ( CFDictionaryRef)[values objectAtIndex:i];
        ...
        for (NSString* member in fields) {
            ...
            if (abKey) {
                ...
                if (testString != nil) {
                    BOOL bExists =  CFDictionaryGetValueIfPresent(dict, ( const void*)abKey, ( const void**)&abValue);
                    if (bExists) {
                        CFRetain(abValue);
                        NSPredicate* containPred = [NSPredicate predicateWithFormat:@"SELF LIKE[cd] %@", testString];
                        bFound = [containPred evaluateWithObject:( id)abValue];
                        CFRelease(abValue);
                    }
                }
            }
