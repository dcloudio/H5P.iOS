...
@property (nonatomic, strong) GCanvasModuleCallback  lastDrawCallback;
 ...
 *
 * @param   data    render commands from js
 * @param   componentId GCanvas component identifier
 */
- (void)render:(NSArray *)data componentId:(NSString*)componentId callback:(GCanvasModuleCallback)callback {
    if( self.enterBackground ) return;
    NSString *commands = [data objectAtIndex:0];
    BOOL hasCallback = false;
    if ( [commands length] > 1 ) {
        hasCallback = [[data objectAtIndex:1] boolValue];
    }
    ...
    if ( hasCallback ) {
        self.lastDrawCallback = callback;
    }
}
...
- (void)refreshPlugin:(GCanvasPlugin*)plugin withComponent:(id<GCanvasViewProtocol>)component{
    ...
    CGRect gcanvasFrame = CGRectMake(compFrame.origin.x, compFrame.origin.y,
                                     ceilf(compFrame.size.width*devicePixelRatio),
                                     ceilf(compFrame.size.height*devicePixelRatio));
...
- (void)glkView:(GLKView*)view drawInRect:(CGRect)rect{
    ...
    dispatch_async([self gcanvasExecuteQueue], ^{
        GCVWeakSelf
        if ( weakSelf.lastDrawCallback ) {
            weakSelf.lastDrawCallback(@{});
            weakSelf.lastDrawCallback = nil;
        }
    });
}
#pragma mark - dcloud method
- (NSDictionary*)measureText:(NSArray*)data componentId:(NSString*)componentId;{
    ...
    if( component && plugin ){
        NSString *text = data[0];
        NSString *fontStyle = data[1];
        float width = [plugin measureText:text fontStyle:fontStyle];
        if ( width >= 0 ) {
            return @{@"width":@(width)};
        }
    }
...
- (void)getImageData:(NSArray*)data componentId:(NSString*)componentId callback:(GCanvasModuleCallback)callback {
    GCanvasObject *gcanvasInst = self.gcanvasObjectDict[componentId];
    id<GCanvasViewProtocol> component = gcanvasInst.component;
    GCanvasPlugin *plugin = gcanvasInst.plugin;
    if( component && plugin ){
        [EAGLContext setCurrentContext:component.glkview.context];
        [plugin getImageData:data callback:callback];
    }
}

- (void)putImageData:(NSArray*)data componentId:(NSString*)componentId callback:(GCanvasModuleCallback)callback {
    GCanvasObject *gcanvasInst = self.gcanvasObjectDict[componentId];
    id<GCanvasViewProtocol> component = gcanvasInst.component;
    GCanvasPlugin *plugin = gcanvasInst.plugin;
    ...
    CGFloat w = [PGPluginParamHelper getFloatValue:[data objectAtIndex:2] defalut:0];
    CGFloat h = [PGPluginParamHelper getFloatValue:[data objectAtIndex:3] defalut:0];
    if( component && plugin ){
        NSString *base64Str = [data objectAtIndex:4];
        NSString *command =[NSString stringWithFormat:@"P%d,%d,%d,%d,%d,%d,%@;", (int)x, (int)y, (int)w,(int)h,(int)w,(int)h, base64Str];
        [plugin addCommands:command];
        [self execCommandById:componentId];
    }
    callback(@{@"errMsg": @"canvasPutImageData:ok"});
}

- (void)toTempFilePath:(NSArray*)data componentId:(NSString*)componentId callback:(GCanvasModuleCallback)callback {
    GCanvasObject *gcanvasInst = self.gcanvasObjectDict[componentId];
    id<GCanvasViewProtocol> component = gcanvasInst.component;
    GCanvasPlugin *plugin = gcanvasInst.plugin;
    if( component && plugin ){
        [EAGLContext setCurrentContext:component.glkview.context];
        NSString *savePath = [self.deletage dc_getTempRootPath];
        [plugin getData:data toTempFilePath:savePath callback:callback];
    }
}
