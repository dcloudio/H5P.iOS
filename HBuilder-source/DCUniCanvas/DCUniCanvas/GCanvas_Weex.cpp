...
#endif
        if ( sw == 0 ) {
            sw = group.mVecTexture[0]->GetWidth();
        }
        if ( sh == 0 ) {
            sh = group.mVecTexture[0]->GetHeight();
        }
        if ( dw == 0 ) {
            dw = group.mVecTexture[0]->GetWidth();
        }
        if ( dh == 0 ) {
            dh = group.mVecTexture[0]->GetHeight();
        }
    ...
    while (p < end) {
        switch (*p) {
            ...
            case 'd': {
                ...
                const TextureGroup *ptr_group = mTextureMgr.Get(clip.textureID);
                if (NULL == ptr_group) {
                    LOG_D("GCanvas::DrawImage texture %d is NULL", clip.textureID);
                    break;
                }
                ApplyTransform(action.a, action.b, action.c, action.d,
                               action.tx, action.ty);
