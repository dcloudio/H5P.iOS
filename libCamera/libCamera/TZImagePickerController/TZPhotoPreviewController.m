...
@interface DCTZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation DCTZPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [TZImageManager manager].shouldFixOrientation = YES;
    __weak typeof(self) weakSelf = self;
    DCTZImagePickerController *_tzImagePickerVc = (DCTZImagePickerController *)weakSelf.navigationController;
...
- (void)configBottomToolBar {
    ...
    DCTZImagePickerController *_tzImagePickerVc = (DCTZImagePickerController *)self.navigationController;
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop) {
        ...
        [DCTZImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)select:(UIButton *)selectButton {
    DCTZImagePickerController *_tzImagePickerVc = (DCTZImagePickerController *)self.navigationController;
    DCTZAssetModel *model = _models[_currentIndex];
        ...
        for (DCTZAssetModel *model_item in selectedModels) {
            if ([[[TZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[TZImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    DCTZAssetModel *model = selectedModelsTmp[i];
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        UIImage *cropedImage = [DCTZImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [DCTZImageCropManager circularClipImage:cropedImage];
        }
