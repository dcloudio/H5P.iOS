...
@interface DCTZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
@implementation DCTZImagePickerController
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[DCTZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[DCTZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    DCTZPhotoPreviewController *previewVc = [[DCTZPhotoPreviewController alloc] init];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        DCTZPhotoPickerController *photoPickerVc = [[DCTZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[TZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage needFetchAssets:NO completion:^(DCTZAlbumModel *model) {
...
- (void)setPickerDelegate:(id<DCTZImagePickerControllerDelegate>)pickerDelegate {
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (id asset in selectedAssets) {
        DCTZAssetModel *model = [DCTZAssetModel modelWithAsset:asset type:[[TZImageManager manager] getAssetType:asset]];
        ...
        [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<DCTZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[DCTZAlbumCell class] forCellReuseIdentifier:@"TZAlbumCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DCTZAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"TZAlbumCell"];
    DCTZImagePickerController *imagePickerVc = (DCTZImagePickerController *)self.navigationController;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    DCTZPhotoPickerController *photoPickerVc = [[DCTZPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    DCTZAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
