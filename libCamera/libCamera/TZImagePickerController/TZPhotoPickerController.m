...
@interface DCTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation DCTZPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (iOS9Later) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[DCTZImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[DCTZImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    DCTZImagePickerController *tzImagePickerVc = (DCTZImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && iOS8Later && self->_model.isCameraRoll) {
            [[TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(DCTZAlbumModel *model) {
                self->_model = model;
            ...
            if (self->_showTakePhotoBtn || !iOS8Later || self->_isFirstAppear) {
                [[TZImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<DCTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[DCTZAssetCell class] forCellWithReuseIdentifier:@"TZAssetCell"];
...
- (void)previewButtonClick {
    DCTZPhotoPreviewController *photoPreviewVc = [[DCTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            DCTZAssetModel *model = tzImagePickerVc.selectedModels[i];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DCTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        DCTZImagePickerController *tzImagePickerVc = (DCTZImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (DCTZAssetModel *model_item in selectedModels) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                DCTZImagePickerController *tzImagePickerVc = (DCTZImagePickerController *)self.navigationController;
                if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(tz_imagePickerControllerPickOverMaxCount:)]) {
                    [tzImagePickerVc.pickerDelegate tz_imagePickerControllerPickOverMaxCount:tzImagePickerVc];
                }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DCTZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            DCTZVideoPlayerController *videoPlayerVc = [[DCTZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            DCTZGifPhotoPreviewController *gifPreviewVc = [[DCTZGifPhotoPreviewController alloc] init];
...
- (void)pushPhotoPrevireViewController:(DCTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)checkSelectedModels {
    ...
    DCTZImagePickerController *tzImagePickerVc = (DCTZImagePickerController *)self.navigationController;
    for (DCTZAssetModel *model in tzImagePickerVc.selectedModels) {
        [selectedAssets addObject:model.asset];
    }
    for (DCTZAssetModel *model in _models) {
        model.isSelected = NO;
...
- (void)reloadPhotoArrayWithMediaType:(NSString *)mediaType {
    DCTZImagePickerController *tzImagePickerVc = (DCTZImagePickerController *)self.navigationController;
    [[TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:NO completion:^(DCTZAlbumModel *model) {
        self->_model = model;
        [[TZImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<DCTZAssetModel *> *models) {
            [tzImagePickerVc hideProgressHUD];
            
            DCTZAssetModel *assetModel;
            if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            DCTZAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
