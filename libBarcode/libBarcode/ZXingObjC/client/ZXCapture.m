...
//
//- (void)setFocusRange:(AVCaptureAutoFocusRangeRestriction)focusRange {
//    
//    if ( self.input.device.isAutoFocusRangeRestrictionSupported
//        && self.input.device.autoFocusRangeRestriction != focusRange ) {
//        _focusRange = focusRange;
//        [self.input.device lockForConfiguration:nil];
//        self.input.device.autoFocusRangeRestriction = focusRange;
//        self.input.device.focusMode = self.focusMode;
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
  ...
  @autoreleasepool {
    ...
    if (!CGRectIsEmpty(self.scanRect)) {
        CGRect adjustScanRect = self.scanRect;
        CGSize bufferSize = CGSizeMake( CGImageGetWidth(rotatedImage), CGImageGetHeight(rotatedImage));
        CGSize layerSize = _layer.frame.size;
        if ( !CGSizeEqualToSize(layerSize, bufferSize) ) {
            CGFloat zoomX = bufferSize.width/layerSize.width;
            CGFloat zoomY = bufferSize.height/layerSize.height;
            adjustScanRect = CGRectMake(ceil(self.scanRect.origin.x * zoomX),
                                        ceil(self.scanRect.origin.y *zoomY),
                                        ceil(self.scanRect.size.width *zoomY),
                                        ceil(self.scanRect.size.height *zoomY));
        }
        CGImageRef croppedImage = CGImageCreateWithImageInRect(rotatedImage, adjustScanRect);
...
//      
//      NSURL *url = [NSURL fileURLWithPath:[documentsDirectory stringByAppendingPathComponent:@"1.png"]];
      ...
      if (self.delegate) {
        ...
        if (result) {
            if (self.captureToFilename) {
                ...
                CGImageDestinationRef dest = CGImageDestinationCreateWithURL((__bridge CFURLRef)url, (__bridge CFStringRef)@"public.png", 1, nil);
                CGImageDestinationAddImage(dest, self.lastScannedImage, nil);
                CGImageDestinationFinalize(dest);
                CFRelease(dest);
                self.captureToFilename = nil;
            }
