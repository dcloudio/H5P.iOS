 ...
 */
@property (nonatomic, readonly, strong) NSString *rId;

/**
 更新组件的高度
 pixel：单位为px值的高度；
 */
- (void)dc_updateStyleHeightWithPixel:(CGFloat)pixel;

/**
 更新组件的高度
 height：逻辑像素高度，不需要在通过转换了
 */
- (void)dc_updateStyleHeight:(CGFloat)height;
 ...
 */
- (void)dc_updateStyles:(NSDictionary *)newStyles;

/**
 更新组件的宽高
 pixel：单位为px值的高度；
 */
- (void)dc_updateStyleWidthPixel:(CGFloat)width heightPixel:(CGFloat)height;


/**
 判断是否已设置宽或者高，如果没有设置则使用默认值设置宽高

 @param defWidth 宽(px值)
 @param defHeight 高(px值)
 */
- (void)dc_setDefaultWidthPixel:(CGFloat)defWidth defaultHeightPixel:(CGFloat)defHeight;
//-----------↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑---------------------------------------------------------
